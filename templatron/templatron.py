...
class Templatron(object):
    ...
    def build_repo(self, repo, base_branch=None):
        ...
        return Repository(
            name, self.token, gh, self.config.clone_root, self.template, **kwargs
        )
    ...
    def maybe_log_dry_run(self):
        if not self.config.dry_run:
            return
        explainer = (
            "Nothing will be pushed to origin and no PRs will be "
        ...
        try:
            ...
            for repo in update_repos:
                try:
                    ...
                    self.logger.error(f"repo {repo.name} failed with exception: {ex}")
            self.stop()
        except KeyboardInterrupt:
            self.maybe_clean()
            raise
