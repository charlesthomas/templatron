...
@click.option("--template-branch", "-b", help="branch of the template to sync from")
@click.option(
    "--template-config",
    "-t",
    default="templatron.yaml",
...
@click_config_file.configuration_option(provider=click_yaml_provider, implicit=False)
...
@click.pass_context
@click.option(
    "--single-repo", "-1", help="update this repo only; bypass repo list / scanning"
)
@click.option(
    "--cache", "-c", help="don't query the GitHub API; use a cached list of repos"
)
